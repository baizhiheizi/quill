...
- **Payout asset.** Revenue is paid out in `payment.asset_id`. (Cross-asset payment previously went through 4swap and could change the payout asset via `payment.swap_order.fill_asset_id`; that path has been removed — multi-asset payment is now handled by MixPay, which settles in the item's own asset.)
