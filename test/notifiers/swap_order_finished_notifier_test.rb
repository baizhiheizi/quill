...
class SwapOrderFinishedNotifierTest < ActiveSupport::TestCase
  setup do
  ...
  test "deliver creates noticed event and notification records" do
    swap_order = build_swap_order!(state: "completed")

    assert_difference -> { Noticed::Event.count }, 1 do
      assert_difference -> { Noticed::Notification.count }, 1 do
        deliver_notifier!(
          SwapOrderFinishedNotifier,
          record: swap_order,
    ...
    assert_equal "SwapOrderFinishedNotifier", event.type
  ...
  test "message uses swapped_message when state is completed" do
    swap_order = build_swap_order!(state: "completed")
    ...
    assert_includes notification.message,
                    I18n.t("notifiers.swap_order_finished_notifier.notification.swapped")
    assert_includes notification.message, format("%.8f", swap_order.funds)
    assert_includes notification.message, @pay_asset.symbol
    assert_includes notification.message, "->"
    assert_includes notification.message, format("%.8f", swap_order.amount)
    assert_includes notification.message, @fill_asset.symbol
  end

  test "swapped_message joins translation, formatted funds, pay symbol, arrow, formatted amount, and fill symbol with spaces" do
    swap_order = build_swap_order!(state: "completed")
    ...
    assert_equal [
      I18n.t("notifiers.swap_order_finished_notifier.notification.swapped"),
      format("%.8f", swap_order.funds),
      @pay_asset.symbol,
      "->",
      format("%.8f", swap_order.amount),
      @fill_asset.symbol
    ].join(" "), notification.message
  end

  test "message uses swapped_message when state is refunded" do
    swap_order = build_swap_order!(state: "refunded")
    ...
    assert_includes notification.message,
                    I18n.t("notifiers.swap_order_finished_notifier.notification.swapped")
    assert_not_includes notification.message, "rejected"
  end

  test "message uses rejected_message when state is rejected" do
    swap_order = build_swap_order!(state: "rejected")
    ...
    assert_equal I18n.t("notifiers.swap_order_finished_notifier.notification.rejected"), notification.message
  end
  ...
  test "data equals message (PLAIN_TEXT shape) for completed state" do
    swap_order = build_swap_order!(state: "completed")
  ...
  test "data equals message (PLAIN_TEXT shape) for rejected state" do
    swap_order = build_swap_order!(state: "rejected")
