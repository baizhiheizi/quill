...
**Backlog cursor:** next run should consider home stats caching (MEDIUM, Data) or investigate code-level opportunities.

## work in progress

- **2026-07-21 draft PR:** branch `efficiency/stimulus-disconnect-cleanup-20260721`. Two Stimulus disconnect improvements: pause pre-order polling while page hidden (saves ~40 requests/min per backgrounded tab); clear clipboard success timeout on disconnect (prevents stale timer handles across Turbo navigations).
