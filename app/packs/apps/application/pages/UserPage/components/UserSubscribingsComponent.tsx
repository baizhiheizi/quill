...
import {
  ...
  useUserSubscribingConnectionQuery,
} from 'graphqlTypes';
...
export default function UserSubscribingsComponent(props: { uid: string }) {
  const { uid } = props;
  const { t, i18n } = useTranslation();
  const { currentUser } = useCurrentUser();
  moment.locale(i18n.language);
  const { data, loading, fetchMore } = useUserSubscribingConnectionQuery({
    variables: { uid },
  });
