  ...
  scope :order_by_popularity, lambda {
    where('orders_count > ?', 0)
      ...
      .select(
        <<~SQL.squish
          articles.*, 
          (((SUM(orders.value_usd) * 10 + articles.upvotes_count - articles.downvotes_count * 20 + articles.comments_count) / POW(((EXTRACT(EPOCH FROM (now()-articles.created_at)) / 3600)::integer + 1), 2))) AS popularity
        SQL
      )
