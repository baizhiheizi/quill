# frozen_string_literal: true

module Mutations
  class ToggleReadingSubscribeUserActionMutation < Mutations::BaseMutation
    argument :mixin_id, String, required: true

    field :error, String, null: true

    def resolve(params)
      user = User.find_by(mixin_id: params[:mixin_id])
      return { error: '找不到用户' } if user.blank?
      return { error: '不能订阅自己' } if current_user == user

      if current_user.reading_subscribe_user?(user)
        current_user.destroy_action(:reading_subscribe, target: user)
      else
        current_user.create_action(:reading_subscribe, target: user)
        ReadingSubscribeNotification.with(action: current_user.find_action(:reading_subscribe, target: user)).deliver(user)
      end

      { error: nil }
    end
  end
end
