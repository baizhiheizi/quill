...
module Users
  class SubscribeUsersController < Users::BaseController
    ...
    # `UserFieldPreloads#user_field_preloads`) and batch the per-row
    # `current_user.subscribe_user?(user)` action_store check via the
    # shared `Users::BaseController#preloaded_subscribe_user_ids` helper.
    def index
