...
- [ ] T001 [P] Record a pre-change baseline: run `bin/rubocop`, `bun run lint-check`, and `bin/rails test` on the unmodified `002-editorial-ui-redesign` branch (repo root) so later regressions are attributable to this feature
- [ ] T002 [P] Create empty Stimulus controller files `app/javascript/controllers/masthead_controller.js` and `app/javascript/controllers/paywall_fade_controller.js`, and register both in `app/javascript/controllers/index.js`
